...
export default function TopSuppliers({ 
  ...
  // ⭐ Navigation vers le profil avec l'ID
  const handleProfileClick = (userId: string) => {
    if (userId) {
      const currentUserRole = getUserRole();
      const basePath = currentUserRole === 'fournisseur' || currentUserRole === 'provider' ? '/f' : '/c';
            ...
            <div className="flex items-center gap-2.5">
              ...
              >
                <img 
                  src={person.avatar} 
                  alt={person.name} 
                  className="size-7 rounded-full object-cover border border-border/50" 
                />
              </button>
              <div>
                {/* ⭐ Nom cliquable avec ID */}
                <button 
                  onClick={() => handleProfileClick(person.id)}
                  ...
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        size={9} 
                        className={i < Math.floor(person.rating) ? "text-amber-400 fill-amber-400" : "text-border"} 
                      />
                    ))}
                  </div>
