...
export default function AnnuairePage({ type, onBack }: AnnuairePageProps) {
  ...
  const filteredUsers = usersDatabase.filter((user) => {
    ...
    const matchesRating = filters.rating === "all" || user.rating >= parseInt(filters.rating);
    ...
    <div className="w-full min-h-screen bg-neutral p-4 md:p-6 lg:p-8 animate-in fade-in duration-300">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {!isLoading && filteredUsers.length > 0 && (
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              ...
              {" "}{filteredUsers.length === 1 ? "professionnel trouvé" : "professionnels trouvés"}
        ...
        {isLoading ? (
          ...
          <AnnuaireEmptyState searchQuery={searchQuery} onResetFilters={resetFilters} />
        )}
      </div>
    </div>
  );
