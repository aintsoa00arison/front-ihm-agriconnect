      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-16 text-center">
        ...
        <div className="mb-14 space-y-3">
          ...
          <p className="text-input-element text-sm max-w-2xl mx-auto leading-relaxed pt-2">
            Chez OmniAgri, notre mission est de numériser la chaîne d'approvisionnement agricole. 
            Nous créons un environnement sécurisé et transparent.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {features.map((feature, index) => (
            <Card 
              key={index} 
              className="bg-white border border-separator/5 rounded-[24px] shadow-sm hover:shadow-md transition-all duration-300 flex flex-col items-center text-center p-6 space-y-4"
            >
              {/* Icône seule, sans conteneur de fond (bg) */}
              <div className="flex items-center justify-center text-primary shrink-0 transition-transform duration-300 group-hover:scale-105">
                {feature.icon}
              </div>
