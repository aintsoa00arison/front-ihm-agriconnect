...
export default function HelpCenter({ setView }: HelpCenterProps) {
  ...
  // Gestion de la redirection fluide vers le formulaire
  ...
  return (
    <div className="w-full bg-[#F8F9FA] font-sans text-input-element antialiased selection:bg-primary/10 select-none">
      ...
      {/* --- SECTION EN-TÊTE & RECHERCHE (HERO) --- */}
      <div className="w-full bg-gradient-to-b from-[#F3F4F6]/50 to-[#F8F9FA] py-20 px-6 lg:px-16 text-center space-y-6 border-b border-separator/10">
        ...
        <div className="max-w-2xl mx-auto relative mt-8">
          <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-input-element/40 z-10" size={18} />
          <Input 
            type="text"
            placeholder="Rechercher une aide, un guide, une règle de conformité..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full h-auto pl-14 pr-6 py-4 rounded-2xl border border-separator/30 shadow-sm text-sm font-medium focus-visible:ring-primary focus-visible:border-primary bg-white text-label placeholder:text-input-element/40"
          />
        </div>
      </div>

      {/* --- GRILLE DES CATÉGORIES (CARD SHADCN) --- */}
      <div className="max-w-[1440px] mx-auto px-6 lg:px-16 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, index) => (
            <Card 
              key={index}
              className="border border-separator/10 rounded-2xl bg-white shadow-sm hover:shadow-md transition-all group cursor-pointer overflow-hidden"
            >
      ...
      {/* --- SECTION ACCORDÉONS FAQ (ACCORDION SHADCN) --- */}
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {filteredFaqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`faq-${index}`}
                className="border border-separator/20 rounded-2xl bg-white shadow-sm transition-all overflow-hidden data-[state=open]:border-primary/30"
              >
                <AccordionTrigger className="w-full flex items-center justify-between p-6 text-left font-bold text-xs lg:text-sm text-label hover:no-underline hover:bg-[#F8F9FA]/50 transition-colors duration-200 [&[data-state=open]>svg]:text-primary">
                  <span className="pr-4">{faq.question}</span>
                </AccordionTrigger>
                <AccordionContent className="p-6 bg-[#F8F9FA] border-t border-separator/10 text-xs lg:text-sm text-input-element/90 leading-relaxed font-medium">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
      ...
      {/* --- CTA SUPPORT INFÉRIEUR (BOUTON "CONTACTEZ-NOUS" DYNAMIQUE) --- */}
        ...
        <div className="relative overflow-hidden rounded-[24px] bg-primary p-8 lg:p-12 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm">
          ...
          <div className="w-full md:w-auto relative z-10 shrink-0">
            <Button 
              ...
              variant="secondary"
              className="w-full md:w-auto bg-white text-primary hover:bg-[#F3F4F6] px-8 py-3.5 h-auto rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-sm cursor-pointer border-0"
            >
              <Headphones size={14} />
              Contactez-nous
            </Button>
