  ...
  return (
    <>
      <nav className="w-full bg-white/80 backdrop-blur-md sticky top-0 z-50 font-sans border-b border-separator/10 select-none shadow-sm">
        <div className="max-w-360 mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-16 py-4">
          {/* --- LOGO --- */}
          <div
            onClick={() => {
              setView("home");
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <button
                  ...
                  className={`text-left text-lg font-bold py-3 px-4 rounded-lg transition-all duration-300 ${
                    isMenuOpen
                      ? "opacity-100 translate-x-0"
                      : "opacity-0 translate-x-4"
                  } ${isActive ? "text-primary bg-primary/10" : "text-input-element hover:text-primary hover:bg-gray-50"}`}
                >
                  {link.name}
                </button>
              );
